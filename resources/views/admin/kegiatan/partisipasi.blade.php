            ...
            <table class="table table-zebra">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="width: 5%">:</td>
                        <td style="width: 65%">{{ date('d-m-Y', strtotime($kegiatan->tanggal)) }}</td>
                    </tr>
                    <tr>
    ...
    <div class="card-body">
        ...
        <div class="overflow-x-auto">
            <table class="table" id="example">
                ...
                <tbody>
                    @foreach ($kegiatan->partisipasiPromoKegiatan as $item)
                        <tr>
                            ...
                            <td>
                                <div class="flex gap-4">
                                    ...
                                    <button class="bg-green-400 p-1 rounded hover:bg-slate-500"
                                        onclick="modal_presensi{{ $item->kode }}.showModal()">
                                    ...
                                    <dialog id="modal_presensi{{ $item->kode }}" class="modal">
                                        <div class="modal-box w-11/12 max-w-3xl">
                                            <h3 class="text-lg font-bold">Presensi
                                                {{ $item->judul }} !</h3>
                                            <div class="modal-action">
                                                <form method="post" action="{{ url('absensi/' . $item->kode) }}"
                                                    method="POST">
                                                    @csrf
                                                    ...
                                                    <div class="form-control">
                                                        <label class="label cursor-pointer">
                                                            ...
                                                            <input type="radio" name="presensi" value="alpha"
                                                                class="radio checked:bg-red-500" required
                                                                @if ($item->status == 'alpha') @checked(true) @endif />
                                                        </label>
                                                    </div>
                                    ...
                                    <button class="bg-red-400 p-1 rounded hover:bg-slate-500"
                                        onclick="my_modal_4{{ $item->kode }}.showModal()">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    ...
                                    <dialog id="my_modal_4{{ $item->kode }}" class="modal">
                                        <div class="modal-box w-11/12 max-w-3xl">
                                            <h3 class="text-lg font-bold">Anda akan menghapus data partisipasi
                                                {{ $item->nama }} !</h3>
                                            <div class="modal-action">
                                                <form method="post" action="{{ url('partisipasi/' . $item->kode) }}"
                                                    method="POST">
                                                    @csrf
